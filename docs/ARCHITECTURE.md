...
- [Paid vs reimbursed: two independent booleans](#paid-vs-reimbursed-two-independent-booleans)
- [Contas: the bill/receivable assistant](#contas-the-billreceivable-assistant)
- [Tabs via query param](#tabs-via-query-param)
- [Inline quick-create from a form select](#inline-quick-create-from-a-form-select)
- [Guarding against React Strict Mode double-fetch](#guarding-against-react-strict-mode-double-fetch)
- [Schema drift: comparing the real DB against src/types/database.ts](#schema-drift-comparing-the-real-db-against-srctypesdatabasets)
- [PWA: installable icon without an image pipeline](#pwa-installable-icon-without-an-image-pipeline)
...
Every transaction/income/recurring form needs the same three lists — cards, categories, people — to populate its selects. Fetching them redundantly per form (each mounting its own `Promise.all()` of 3 `fetch()` calls) meant opening "Nova transação" fired 3 duplicate GETs even though the data almost never changes between page loads.

### Solution: populate the Context once, at the layout

`src/app/(app)/layout.tsx` is an `async` Server Component. It fetches `cards`, `categories`, and `people` once per request (in parallel via `Promise.all()`) and passes them straight into `TransactionDataProvider`:
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <TransactionDataProvider cards={cards} categories={categories} people={people}>
      {/* Sidebar, Navbar, TransactionPanel, {children} */}
    </TransactionDataProvider>
  )
}
```

Because this runs at the layout level (not a specific page), **every** route under `(app)` gets the same populated Context for free — `TransactionForm`, `IncomeFormDialog`, `RecurringFormDialog`, and `RecurringIncomeFormDialog` all just call `useTransactionData()` and read real data immediately, with zero fetches of their own:

```tsx
// components/transactions/TransactionForm.tsx
const { cards, categories, people } = useTransactionData() // that's it — no useEffect, no loading state
```

`useTransactionData()` itself stays a plain, business-logic-free Context reader:
...
**Why this replaced the earlier per-form fetch-with-fallback pattern:** the previous design had each form fetch locally if the Context was empty (guarded by a `useRef` to dedupe). That was a reasonable client-side patch when the layout only ever passed empty arrays, but it meant every form still needed its own loading state and skeleton (`TransactionFormSkeleton`) just to cover a fetch that didn't need to exist — the layout can just hand over real data synchronously as part of the server render. Once the layout started populating the Context, the client fallback fetch and its skeleton became dead code and were removed.
...
## Paid vs reimbursed: two independent booleans

`installments` has two boolean+timestamp pairs that look similar but mean completely different things:

- **`paid` / `paid_at`** — "I paid my card issuer for this installment." Set from the Invoices page (`/invoices`) and from the Contas assistant.
- **`reimbursed` / `reimbursed_at`** — "The person tagged on this purchase (`transactions.person_id`) paid me back for it." Set from the per-person debt ledger (`/people/[id]`).

These are deliberately decoupled: a purchase can be fully paid to the bank while the third party you fronted it for still owes you, or vice versa in theory. Both endpoints go through the same `PATCH /api/installments/[id]` route (`src/lib/validations.ts`'s `installmentUpdateSchema` accepts `paid` and/or `reimbursed` independently), which resolves whichever field was sent and stamps its own timestamp — never both at once unless the caller explicitly sends both.
...
Why not model "who owes me" as a whole separate table? Because the debt is inherently per-invoice-month — a parceled purchase paid back over 10 months should be reimbursable one installment at a time, mirroring how `paid` already works per-installment for the same reason.

---

## Contas: the bill/receivable assistant

`transactions.status` (`posted`/`scheduled`/`cancelled`) already modeled "this hasn't happened yet" for expenses — but nothing surfaced it anywhere, and `income` had no equivalent concept at all. Contas closes that gap without introducing a new "type" of record: it's a read model over data that already exists.

**Schema additions (income only — transactions already had what was needed):**
```sql
alter table public.income
  add column status        text not null default 'received' check (status in ('scheduled', 'received')),
  add column scheduled_for date,
  add column received_at   timestamptz;
```

**What counts as a "conta" (`GET /api/contas`):**
- expenses: `status = 'scheduled'`, or `status = 'posted'` with an unpaid installment — restricted to `installments_count = 1`. Multi-installment credit purchases stay exclusively in `/invoices`; a bill assistant isn't the place to manage a 10x TV purchase.
- income: any row with `status = 'scheduled'`.

**Marking a scheduled bill "paid" in one call** (`PATCH /api/transactions/[id]/pay`) has to do two things atomically that the client can't sequence itself (it doesn't know the installment id yet, since a scheduled transaction has none):
1. Generate the installment(s) via the same `generateInstallments()` helper used everywhere else, inserted already as `paid: true`.
2. Flip the transaction to `status: 'posted'`, **and set `scheduled_for: null`** — the database enforces a check constraint (`transactions_scheduled_columns_check`) that `scheduled_for` must be null whenever `status = 'posted'`. This was actually a latent bug already present in `process-transactions/route.ts` (it flips scheduled→posted without nulling `scheduled_for`), only caught once the live schema was dumped and diffed against the code — see [Schema drift](#schema-drift-comparing-the-real-db-against-srctypesdatabasets).

Income's "mark received" doesn't need a dedicated endpoint — since income has no child rows like installments, the existing generic `PATCH /api/income/[id]` already resolves `status`/`scheduled_for`/`received_at` server-side, so the client just sends `{ status: 'received' }`.

**Dashboard numbers** (`getBillsSummary()` in `src/components/dashboard/queries.ts`) intentionally scope "to pay"/"to receive" to *outstanding regardless of month* (current + overdue), not a strict calendar month like the rest of the dashboard — a bills assistant that hides an overdue bill because the calendar rolled over defeats its own purpose. "Already paid"/"already received" stay scoped to the current month, mirroring `expensesPaid`.

---

## Tabs via query param

Contas (Pendentes/Concluídas), Recorrências (contas recorrentes/receitas recorrentes), and Lançamentos (Saídas/Entradas) all use the same lightweight tab pattern instead of local `useState`:

```tsx
// components/contas/ContasTabs.tsx (same shape as RecurringTabs.tsx, PersonLedgerTabs.tsx)
'use client'
export function ContasTabs({ status }: { status: string }) {
  const pathname = usePathname()
  const searchParams = useSearchParams()

  function buildHref(newStatus: string) {
    const params = new URLSearchParams(searchParams.toString())
    if (newStatus === 'pending') params.delete('status') // default tab has no query param
    else params.set('status', newStatus)
    return `${pathname}${params.toString() ? `?${params}` : ''}`
  }

  return (
    <Link href={buildHref('pending')} className={/* active styles if status !== 'done' */}>Pendentes</Link>
    // ...
  )
}
```

**Why a URL param instead of `useState`:** the tab is shareable/bookmarkable (`/recurring?tab=income`), survives a refresh, and — because it's a real navigation via `<Link>` — the Server Component page re-runs with the new `searchParams`, so server-fetched data (summary tiles, counts) updates along with the tab, not just the client list underneath it.

---

## Inline quick-create from a form select

Filling out a transaction and realizing the category doesn't exist yet used to mean abandoning the form, creating the category on `/categories`, and starting over. Every `Select` for card/category/person now has a `+ Novo X` item at the bottom that opens the existing CRUD dialog (`CardFormDialog`/`CategoryFormDialog`/`PeopleFormDialog`) **on top of** the current form, without closing it:
...
>
  {/* ...categoryOptions.map(...) */}
  <SelectItem value="__new_category__">+ Nova categoria</SelectItem>
</Select>
...
<CategoryFormDialog
  ...
  onSaved={(created) => {
    if (!created) return
    setExtraCategory(created)                                    // 1. optimistic: show it immediately
    form.setValue('category_id', created.id, { shouldValidate: true }) // 2. auto-select it
  }}
/>
```

Two things make this work without touching `TransactionDataProvider`'s architecture:

1. **The CRUD dialogs now return what they created.** `onSaved` used to be `() => void`; it's now `(saved?: Card) => void`, and each dialog parses the POST/PATCH response body and passes it through. Existing callers that ignore the argument still compile (TypeScript allows a narrower callback to be passed where a wider one is expected).
2. **A one-render optimistic merge, not a Context mutation.** The dialog's existing `router.refresh()` (already there for every CRUD dialog, unrelated to this feature) re-runs the layout and eventually refreshes `TransactionDataProvider` with the real list — but that's a network round trip away. In the meantime, the form keeps a small local `extraCategory` state and merges it into the options list, deduped by id once the real list catches up:
   ```ts
   const categoryOptions = extraCategory && !categories.some(c => c.id === extraCategory.id)
     ? [...categories, extraCategory]
     : categories
   ```
   This is deliberately per-form, local state — not a shared cache — because the moment `categories` (from Context) includes the new id, the local copy becomes redundant and is naturally ignored.

Stacking a `Dialog` on top of another open `Dialog` (or on top of the sliding `TransactionPanel`) works with zero extra z-index handling: every `DialogContent` in `src/components/ui/dialog.tsx` uses the same `z-50`, and Radix mounts each into its own portal appended later in the DOM — later-painted wins the stacking order for free.

---

## Guarding against React Strict Mode double-fetch

`RecurringList`, `RecurringIncomeList`, `ContasList`, and `PersonLedgerList` all fetch their own page of data in a `useEffect`. In development, React 18 Strict Mode (on by default with the App Router) intentionally mounts, runs effects, unmounts, and re-mounts once — which means every real mount (e.g. clicking a tab, which unmounts the previous list and mounts the new one) fired the same GET twice.

```ts
const fetchedPageRef = useRef<number | null>(null) // or a composite key like `${status}:${page}`
useEffect(() => {
...
The ref survives Strict Mode's mount → effect → cleanup → effect replay (refs aren't reset by re-running effects, only by a real unmount), so the second synthetic invocation short-circuits. This has zero effect on production (Strict Mode's double-invoke never happens in `next build`/`next start`) — it only silences noise in `next dev`, and as a side effect also protects against any future legitimate double-render from firing a redundant request.

---

## Schema drift: comparing the real DB against `src/types/database.ts`

There's no migration tooling in this repo — schema changes are applied by hand via the Supabase SQL Editor (see `docs/database/schema.sql`'s generation instructions in the README), and `src/types/database.ts` is maintained by hand alongside them. That gap is exactly how three real bugs slipped in and went unnoticed until the schema was dumped and diffed line-by-line against the code that writes to it:

1. **`cards`, `categories`, `people`, `transactions`, and `installments` don't have an `updated_at` column** in the real database — only `income`, `recurring_income`, and `recurring_transactions` do. `database.ts` declared `updated_at` on all of them. Harmless as long as nothing ever set it — until new code (the Contas `/pay` endpoint) did, which would have thrown "column does not exist" the first time anyone marked a scheduled bill as paid.
2. **`installments.paid_at` didn't exist at all**, despite being declared in `database.ts` since long before this feature — new code (Contas' `/pay` endpoint and the `/api/contas` "Concluídas" tab) assumed it did. Fixed by actually adding the column and wiring `PATCH /api/installments/[id]` to set it (mirroring how `reimbursed_at` already worked).
3. **`transactions_scheduled_columns_check`** (a real constraint in the live DB) requires `scheduled_for IS NULL` whenever `status = 'posted'` — a rule that `process-transactions/route.ts`'s scheduled→posted step had been silently violating since before this round of changes.

**Takeaway:** when adding a column that new code depends on, generate a fresh dump (`npx supabase db dump --schema public -f docs/database/schema.sql`) and grep the target table before assuming a type in `database.ts` reflects reality — the type file has no automated guarantee of staying in sync with the database it describes.

---

## PWA: installable icon without an image pipeline

Making the app installable (home-screen icon, standalone display, no browser chrome) needs a `manifest.ts`/`manifest.webmanifest`, an `apple-touch-icon` for iOS, and PNG icons at a couple of sizes for Android — normally that means running actual image files through a design tool or a library like `sharp`. Instead, every icon here is generated at request/build time with `next/og`'s `ImageResponse`, using plain flexbox JSX (no raster asset checked into the repo at all):

```tsx
// app/apple-icon.tsx — Next.js special-file convention, auto-linked in <head>
...
export default function AppleIcon() {
  return new ImageResponse(
    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'flex-end', gap: 14, background: '#EFF6FF' }}>
...
The 3 bars reuse the exact palette from the existing SVG favicon (a rising bar-chart mark), just rebuilt as divs instead of trying to rasterize the SVG through Satori's limited `<img>` support. `apple-icon.tsx` is a Next.js special file (auto-served at `/apple-icon` and auto-linked); `icon-192`/`icon-512` are plain Route Handlers returning the same `ImageResponse` at different sizes, referenced explicitly from `manifest.ts`'s `icons` array since Next's file-convention icons only cover one size at a time. Scope was deliberately limited to installability — no service worker or offline cache was added, so the app still requires network access exactly like the browser version.
