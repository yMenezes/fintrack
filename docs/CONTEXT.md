...
8. ✅ FASE 6 — Dashboard com Gráficos
9. ✅ FASE 7 — Gastos Futuros (status posted/scheduled/cancelled)
10. ✅ FASE 8 — Income + recorrência de receita, limpeza de fetches duplicados
11. ✅ FASE 9 — Contas: assistente de contas a pagar/receber
12. ✅ FASE 10 — Extrato de dívida por pessoa / reembolso
13. ✅ FASE 11 — Cadastro rápido inline nos formulários
14. ✅ FASE 12 — PWA, 404 customizado, reorganização do dashboard
15. ✅ FASE 13 — Supabase CLI + auditoria de schema
...
### ✅ FASE 8 — Income + Recorrência de receita, e limpeza de fetches duplicados (PRONTO)

#### Fix de build quebrado
- `recurringIncomeCreateSchema`/`recurringIncomeUpdateSchema` faltavam em `lib/validations.ts` (route.ts e o form dialog já importavam, mas o export não existia)
- `src/app/api/recurring-income/[id]/route.ts` (PATCH/DELETE) não existia — form de edição e exclusão retornavam 404. Criado seguindo exatamente o padrão de `recurring-transactions/[id]/route.ts`

#### Data loading: layout popula o Context no servidor
- `src/app/(app)/layout.tsx` virou Server Component assíncrono — busca `cards`/`categories`/`people` uma vez via `Promise.all()` e passa pro `TransactionDataProvider`, em vez dos arrays vazios de antes
- `TransactionForm.tsx` parou de fazer fetch client-side de fallback (removido `fetchedRef`, `isLoadingData`, e o componente `TransactionFormSkeleton.tsx`, que ficou órfão)
- Resultado: abrir qualquer formulário de lançamento não dispara mais `/api/cards`, `/api/categories`, `/api/people`

#### Abas de Recorrências
- `RecurringIncomeList` existia mas não estava integrado em nenhuma tela — criado `RecurringTabs.tsx` (mesmo padrão de tabs por query param já usado em Lançamentos) e `/recurring?tab=income` alterna entre contas recorrentes e receitas recorrentes

#### Double-fetch em dev (React Strict Mode)
- `RecurringList`/`RecurringIncomeList` disparavam 2 GETs idênticos por montagem em `next dev` (Strict Mode roda os efeitos duas vezes de propósito) — guarda via `useRef` (`fetchedPageRef`) resolve, sem efeito em produção

### ✅ FASE 9 — Contas: assistente de contas a pagar/receber (PRONTO)

- `income` ganhou `status` (`scheduled`/`received`), `scheduled_for`, `received_at` — mesmo padrão de `transactions.status`/`scheduled_for`. Toggle "Agora/Agendar" adicionado no `IncomeFormDialog`
- Novo `PATCH /api/transactions/[id]/pay` — transforma uma transação agendada em paga numa chamada só (gera parcela + marca paga), ou alterna `paid` de uma transação já postada
- Novo `GET /api/contas` — lista unificada de pendências/concluídas (gasto single-installment + receita), ordenada por vencimento
- Nova página `/contas` (abas Pendentes/Concluídas), novo card "Contas" no dashboard (a pagar/pago/a receber/recebido, com total vencido) e link no menu
- `PaidCheckbox` extraído de `InvoiceInstallmentRow` pra reaproveitar no checkbox de pago/recebido
- Fora de escopo, por decisão: compras parceladas no crédito continuam só em `/invoices`; notificação de vencimento fica pra depois (sem infra de cron/push ainda)

### ✅ FASE 10 — Extrato de dívida por pessoa / reembolso (PRONTO)

- `installments` ganhou `reimbursed`/`reimbursed_at` — conceito **separado** de `paid`/`paid_at` (pagar o banco ≠ a pessoa te reembolsar)
- `PATCH /api/installments/[id]` estendido pra aceitar `paid` e/ou `reimbursed` independentemente
- Nova página `/people/[id]`: 3 tiles (total devido/vencido/deste mês) + lista Pendentes/Reembolsadas somando parcelas de todos os cartões da pessoa, marcando vencidas as com mês de referência já passado
- Acessível pelo ícone de carteira em cada linha de `/people`
- Fora de escopo: só considera transações já postadas (agendada ainda não é dívida de ninguém)

### ✅ FASE 11 — Cadastro rápido inline nos formulários (PRONTO)

- `CardFormDialog`/`CategoryFormDialog`/`PeopleFormDialog` agora devolvem a entidade criada via `onSaved(entity)` (antes `onSaved()` não levava nada)
- `TransactionForm`, `IncomeFormDialog`, `RecurringFormDialog`, `RecurringIncomeFormDialog` ganharam um item "+ Novo cartão/categoria/pessoa" no fim de cada select, que abre o diálogo por cima do formulário atual (empilhado, sem fechar nada) e seleciona o item assim que criado — merge otimista local, sem mudar a arquitetura do `TransactionDataProvider`

### ✅ FASE 12 — PWA, 404 customizado e reorganização do dashboard (PRONTO)

- `manifest.ts`, `apple-icon.tsx`, `icon-192`/`icon-512` (gerados via `next/og`, sem pipeline de imagem) — app instalável na tela inicial, `appleWebApp` + `viewport.themeColor` no layout raiz
- `not-found.tsx` customizado com a identidade visual do app
- Dashboard reordenado: Cards de resumo → Distribuição por categoria + Contas → Fluxo de caixa → Movimentações recentes + Resumo do mês + Próximos lançamentos recorrentes (3 colunas iguais)

### ✅ FASE 13 — Supabase CLI + auditoria de schema (PRONTO)

- Supabase CLI instalado como devDependency, projeto linkado (`supabase link`), `supabase db dump` gera `docs/database/schema.sql` (gitignorado) a partir do banco real
- Comparação schema real × `src/types/database.ts` achou 3 bugs reais (2 introduzidos nesta rodada, 1 pré-existente em `process-transactions`):
  1. `updated_at` não existe em `cards`/`categories`/`people`/`transactions`/`installments` — código novo gravava nela, removido dos tipos e do código
  2. `installments.paid_at` não existia de verdade no banco — coluna adicionada, e `PATCH /api/installments/[id]` agora grava de verdade (igual já fazia com `reimbursed_at`)
  3. `scheduled_for` não estava sendo zerado ao postar uma transação agendada, violando a constraint `transactions_scheduled_columns_check` — corrigido em `/pay` e no `process-transactions` (bug pré-existente, achado só com o schema real em mãos)
- Achado pendente de correção no banco (não é bug de código): `income_source_check` aceita `'loan'` e rejeita `'investment'`, o app faz o oposto — SQL de correção entregue, ainda não aplicada
...
## Próximos passos (estado atual — pré-teste com usuário real)

**Pendente aplicar no banco (SQL já entregue, não é código):**
1. `income_source_check` — corrigir pra aceitar `'investment'` em vez de `'loan'` (mismatch achado na FASE 13)

**Fora de escopo, decidido conscientemente:**
1. Notificação de vencimento (Contas) — precisa de infra de cron/push que não existe hoje
2. Compras parceladas no crédito dentro de Contas — ficam só em `/invoices`, de propósito

**Ainda no roadmap:**
1. Exportação CSV
2. Revisão de segurança
3. Testar com usuário real (em andamento agora — dois usuários de teste apagados pra recomeçar validação do zero)
